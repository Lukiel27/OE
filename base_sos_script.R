...
missingness <- 0.3
species   <- "OE" # folder name
dataset   <- "DOre22-7415" # report name

basedir <- ""

d1        <- new.read.dart.xls.onerow(RandRbase,species,dataset,topskip, nmetavar, euchits=FALSE)
keep <- d1$sample_names
keep <- keep[!(keep %in% c('MC_B4','MC_B6'))]
d2 <- remove.by.list(d1, keep)
...
kin <- individual_kinship_by_pop(dms, RandRbase, species, dataset, dms$meta$analyses[,"sp"], maf=0.1, mis=0.3, as_bigmat=TRUE)
...
dist_matrix <- as.matrix(dist(d1$gt, diag=TRUE))
Heatmap(dist_matrix)
...
fst_stats <- faststats(dms$gt, 
                       ...
                       site_variable = dms$meta$analyses[,"sp"], 
                       max_missingness=0.3)
...
v <- diveRsity::divMigrate(infile=c, outfile=NULL, stat="gst",plot_network=TRUE, filter_threshold = 0, boots=100, para=TRUE)
